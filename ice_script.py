...
structure = read_gro(f'ice/ice.gro')
...
filename = 'ice_'+ mol_names + '_' + mol_nums
...
with open(f'systems/{folder}/system.itp', 'w') as f:
    ...
    f.write('#include "tip4p.itp"\n')

    f.write(f'\n[ system ]\n{filename}\n')
    f.write('\n[ molecules ]\n; molecule name\tnr.\n')
    f.write('#include "icemol.itp"\n')
...
gmx grompp -f nvt_ice_steep.mdp -c {filename}_init.gro -p {ff}.top -o {filename} -maxwarn 10
prun gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -ntomp 2 -gpu_id 0
rm ./#*#
rm *pdb

gmx grompp -f nvt_ice_short.mdp -c {filename}.gro -p {ff}.top -o {filename} -maxwarn 10
prun gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -ntomp 2 -gpu_id 0
rm ./#*#

gmx grompp -f nvt_ice_run.mdp -c {filename}.gro -p {ff}.top -o {filename} -maxwarn 10
prun gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -ntomp 2 -gpu_id 0
rm ./#*#
    ''')
...
os.system(f'scp {dir}/itp/tip4p.itp {server_name}:{args.path}/{folder}')
os.system(f'scp ice/icemol.itp {server_name}:{args.path}/{folder}')

os.system(f'scp {dir}/{ff}.top {server_name}:{args.path}/{folder}/{ff}.top')

files = ['nvt_ice_steep.mdp', 'nvt_ice_short.mdp', 'nvt_ice_run.mdp']
for file in files:
