...
structure = read_gro(f'muscovite/musc.gro')
...
names = ['decane', 'tip4p', 'sodium_ClayFF']
...
insert_shapes = [box_left_delta, box_right_delta, box_right_delta]
shapes = [box_left, box_right, box_right]
...
filename = 'musc_'+ mol_names + '_' + mol_nums
...
with open(f'systems/{folder}/system.itp', 'w') as f:
    ...
    f.write('#include "musc.itp"\n')

    f.write(f'\n[ system ]\n{filename}\n')
    f.write('\n[ molecules ]\n; molecule name\tnr.\n')
    f.write('#include "muscmol.itp"\n')
    for i, name in enumerate(names[:-1]):
        f.write(f'{name}\t{numbers[i]}\n')
    f.write(f'sodium\t{numbers[-1]}\n')
...
gmx grompp -f nvt_musc_steep.mdp -c {filename}_init.gro -p {ff}.top -o {filename} -maxwarn 10
prun gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -ntomp 2 -gpu_id 0
rm ./#*#
rm *pdb

gmx grompp -f nvt_musc_short.mdp -c {filename}.gro -p {ff}.top -o {filename} -maxwarn 10
prun gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -ntomp 2 -gpu_id 0
rm ./#*#

gmx grompp -f nvt_musc_run.mdp -c {filename}.gro -p {ff}.top -o {filename} -maxwarn 10
prun gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -ntomp 2 -gpu_id 0
rm ./#*#
    ''')
...
files = ['tip4p.itp', 'decane.itp', 'sodium_ClayFF.itp']
for file in files:
    os.system(f'scp {dir}/itp/{file} {server_name}:{args.path}/{folder}')
os.system(f'scp muscovite/musc.itp {server_name}:{args.path}/{folder}')
os.system(f'scp muscovite/muscmol.itp {server_name}:{args.path}/{folder}')

os.system(f'scp {dir}/{ff}.top {server_name}:{args.path}/{folder}/{ff}.top')

files = ['nvt_musc_steep.mdp', 'nvt_musc_short.mdp', 'nvt_musc_run.mdp']
for file in files:
